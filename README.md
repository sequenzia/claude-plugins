...
```
PRD Creation → Task Generation → Implementation → Release
     ↓               ↓                 ↓            ↓
 prd-tools    task-manager/      dx-tools      dx-tools
              mission-control   (git-commit)   (release)
```

1. **prd-tools → task-manager**: PRDs generated by prd-tools can be analyzed to create task lists
2. **task-manager → dx-tools**: Completed tasks committed using git-commit, changelog updated
3. **dx-tools**: Full release pipeline with changelog, versioning, and tagging
