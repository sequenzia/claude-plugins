...
- `--no-auto-sync`: Disable automatic task status updates (default: auto-sync enabled)
...
AUTO_SYNC = --no-auto-sync flag NOT present (default: true)
...
```bash
# Basic sequential execution (auto-sync enabled by default)
/ralph-task-executor:execute tasks/my-feature.tasks.json

# Parallel execution
/ralph-task-executor:execute tasks/my-feature.tasks.json --parallel

# Disable auto-sync for manual control
/ralph-task-executor:execute tasks/my-feature.tasks.json --no-auto-sync
